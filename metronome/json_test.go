...
var _ = Describe("JSON rendering", func() {
	...
	It("Makes a job with hidden API field", func() {
		...
		Expect(job2.History_).ShouldNot(Equal(nil))
		Expect(job2.HistorySummary_).ShouldNot(Equal(nil))
		Expect(job2.HistorySummary_.SuccessCount).Should(Equal(19))
		Expect(job2.HistorySummary_.FailureCount).Should(Equal(1))
		Expect(len(job2.History_.SuccessfulFinishedRuns)).To(Equal(19))
		Expect(len(job2.History_.FailedFinishedRuns)).To(Equal(1))
